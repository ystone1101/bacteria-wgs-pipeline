...
| `GENOME_SIZE`, `ASM_COVERAGE` | long_np/long_pb | caps Flye's initial assembly coverage (default `5m`/`50`); see [Notes](#notes) |
...
- Flye can fail with `ERROR: No disjointigs were assembled` on very high
  per-sample coverage (seen around 600x+, documented at >800x — e.g. a
  small genome sequenced deep on a high-output flow cell). `config.sh`
  ships with `GENOME_SIZE="5m"`/`ASM_COVERAGE="50"` by default for exactly
  this reason, capping the reads Flye uses for its initial assembly
  stage on every `long_np`/`long_pb` sample; the full read set is still
  used later for repeat resolution and polishing, so this doesn't drop
  plasmids or small contigs. `GENOME_SIZE` doesn't need to be precise — a
  rough same-order estimate is fine, and erring a little high is safer
  than a little low; override per-sample with `-g <genome size>
  --asm-coverage N` (see -h) if 5m is a poor fit for what you're
  sequencing, or set both to `""` in `config.sh` to disable the cap
  entirely and let Flye use every read.
